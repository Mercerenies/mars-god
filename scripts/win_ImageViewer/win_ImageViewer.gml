...
function ImageViewer() : WindowContents() constructor {
  _file_name = new _ImageViewer_FileNameBox(self);
  _open_button = new _ImageViewer_OpenButton(self);
  _image = noone;
...
function ImageViewerEntry() : StartMenuEntry() constructor {
  ...
  static onClick = function() {
    Windows.addOrFindWindow(new ImageViewer(), 201, 96, -1, -1);
  }

}

function _ImageViewer_FileNameBox(owner) : Textbox() constructor {
  _owner = owner;
...
function _ImageViewer_OpenButton(owner) : SpriteButton() constructor {
  _owner = owner;
